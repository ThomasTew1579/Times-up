...
export default function CustomGameSetup() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    navigate(
      `/game/custom/cards/setup?duration=${duration}&players=${players}&playerNames=${namesParam}`
    );
  }
    ...
    <main className="mx-auto container max-w-xl px-4 py-8">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="space-y-2">
          <label htmlFor="duree" className="block text-sm font-medium text-white font-primary">
            Durée du tour
          </label>
          <select
            id="duration"
        ...
        <div className="space-y-2">
          <label htmlFor="players" className="block text-sm font-medium text-white font-primary">
            Nombre de joueurs
          </label>
          <input
            id="players"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {playerNames.map((name, idx) => (
              <input
                ...
                value={name}
                onChange={(e) =>
                  setPlayerNames((arr) => arr.map((n, i) => (i === idx ? e.target.value : n)))
                }
                className="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-primary-900 dark:text-white px-3 py-2 text-sm"
